...
#define SDT_OSC_DICT_DECLARE(TYPE, NAME) \
  json_value *dict_##TYPE = json_object_new(0);

#define SDT_OSC_SAVER_SWITCH(TYPE, NAME)          \
  if ((v = CONCAT(SDT_get, SDT_##TYPE)(argv[i]))) \
    json_object_push(dict_##TYPE, argv[i],        \
                     CONCAT(SDT_TYPE_FULL(SDT_##TYPE), _toJSON)(v));

#define SDT_OSC_DICT_PUSH(TYPE, NAME)                  \
  push_else_free(prj, STRINGIFY(NAME##s), dict_##TYPE, \
                 dict_##TYPE->u.object.length);
...
#define SDT_OSC_LOADER(TYPE, NAME)                                         \
  static int CONCAT(SDTProject_load, SDT_##TYPE)(                          \
      const json_object_entry *value, SDTOSCReturnCode *return_code,       \
    ...
    if (!SDTProject_assert(value->value->type == json_object, return_code, \
        ...
        !SDTProject_assert(                                                \
            (uintptr_t)(x = CONCAT(SDT_get, SDT_##TYPE)(value->name)),     \
            return_code, return_msg, SDT_OSC_RETURN_OBJECT_NOT_FOUND,      \
            value->name, json_string_new("not found")))                    \
      return 0;                                                            \
    SDT_TYPE_FULL(SDT_##TYPE) *tmp =                                       \
        CONCAT(SDT_TYPE_FULL(SDT_##TYPE), _fromJSON)(value->value);        \
