...
void _TestHelper__SDT_argExtremum(CuTest* tc, unsigned int (*find_func)(double *, unsigned int, double *), double (*get_func)(double *, unsigned int), int gt)
{
  ...
  FOR_RANDOM_ITER_INT(sizes, s)
  {
    ...
    CuAssertDblEquals_Msg(tc, "Check find_func and get_func match", get_func(a, s), m, 0.0);
    FOR_RANDOM_ITER_FLOAT(values, v)
      CuAssert(tc, "Check comparison with other values", (gt)? m >= v : m <= v);
...
void TestSDT_bitReverse(CuTest* tc)
{
  ...
  {
    max = 1 << n_bits;
    SDTRandomSequence *x = SDTRandomSequence_newInt((n_bits < 6)? max : n_bits * 9 , 0, max - 1);
    FOR_RANDOM_ITER_INT(x, i)
      CuAssertIntEquals(tc, i, SDT_bitReverse(SDT_bitReverse((unsigned int) i, n_bits), n_bits));
    SDTRandomSequence_free(x);
  }
  SDT_TEST_END()
}
...
void TestSDT_gravity(CuTest* tc)
{
  ...
  FOR_RANDOM_ITER_FLOAT(x, f)
    CuAssertDblEquals_Msg(tc, "Check inverse formula", f, SDT_gravity(f) / SDT_EARTH, 0.000000001);
  SDTRandomSequence_free(x);
  SDT_TEST_END()
}
...
void TestSDT_haar__SDT_ihaar(CuTest* tc)
{
  ...
  {
    for (i = 0; i < s; ++i)
      x[i] = SDTRandomSequence_nextFloat(values);
...
void _TestHelper__SDT_isExtremum(CuTest* tc, double baseval, double peakval, int (*check_func)(double *, unsigned int, unsigned int))
{
  unsigned int size = 1024, i, j, t, r_i, n, left, right;
  int r[size];
  double a[size];
  char msg[128], fmt[] = "Check radius=%i (%i/%i) is %srespected %i <- %i -> %i [%f <- %f -> %f]", *msg2;
  ...
  for (i = 0; i < 64; ++i)
  {
    // Generate extrema
    a[0] = peakval;
    for (j = 1, t = 1 + (r[n = 0] = SDTRandomSequence_nextInt(radii)); j < size; ++j)
    {
      if (j == t)
      {
    ...
    // Check extrema
    for (t = 1 + r[r_i = 0]; r_i < (n - 1) && t < size; ++r_i, t += 1 + ((r[r_i] > r[r_i - 1])? r[r_i] : r[r_i - 1]))
    {
      left = t - r[r_i];
      right = t + r[r_i];
      ...
      CuAssertIntEquals_Msg(tc, msg2, r[r_i], find_func(a + left, 2 * r[r_i] + 1, 0));
      sprintf(msg, fmt, r[r_i], r_i + 1, n, "", left, t, right, a[left], a[t], a[right]);
      // SDT_DEBUG_LOGA(_SDT_eprintf, "%s\n", msg);
      CuAssert(tc, msg, check_func(a, t, r[r_i]));
      sprintf(msg, fmt, r[r_i] + 1, r_i + 1, n, "not ", left - 1, t, right + 1, a[left - 1], a[t], a[right + 1]);
      // SDT_DEBUG_LOGA(_SDT_eprintf, "%s\n", msg);
...
{
  SDT_TEST_BEGIN()
  CuAssertIntEquals_Msg(tc, "Check same debug flag between test and library", SDT_DEBUG_IF_ELSE(1, 0), SDT_isDebug());
  SDT_TEST_END()
}
...
void TestSDT_kinetic(CuTest* tc)
{
  ...
  FOR_RANDOM_ITER_FLOAT(masses, m)
  {
    ...
    CuAssertDblEquals_Msg(tc, "Check quadratic dependency on velocity", e1, e2, tol);
  }
...
void TestSDT_normalize(CuTest* tc)
{
  ...
  FOR_RANDOM_ITER_INT(sizes, s)
  {
    ...
    x_max_1 = SDT_max(a, s);
    for (i = 0; i < s; ++i)
      a[i] = SDT_normalize(a[i], left, right);

    x_min_2 = SDT_min(a, s);
    sprintf(msg, fmt, "Minimum", x_min_2);
    CuAssert(tc, msg, x_min_2 >= 0);
    CuAssertDblEquals_Msg(tc, "Minimum consistency", (x_min_1 - left) / (right - left), x_min_2, 0.0);
    x_max_2 = SDT_max(a, s);
    sprintf(msg, fmt, "Maximum", x_max_2);
    CuAssert(tc, msg, x_max_2 <= 1);
    CuAssertDblEquals_Msg(tc, "Maximum consistency", (x_max_1 - left) / (right - left), x_max_2, 0.0);
  }
  SDTRandomSequence_free(sizes);
  SDTRandomSequence_free(values);
  SDT_TEST_END()
...
void TestSDT_normalizeWindow(CuTest* tc)
{
  ...
  FOR_RANDOM_ITER_INT(sizes, s)
  {
    ...
    CuAssertDblEquals_Msg(tc, "Check sum", 1.0, SDT_average(a, s) * s, 0.000000001);
  }
  SDTRandomSequence_free(sizes);
  SDTRandomSequence_free(values);
  SDT_TEST_END()
...
void _TestHelper__SDT_fill(CuTest* tc, void (*fill_func)(double *, int), float target_value, float random_min, float random_max, float tol)
{
  const unsigned int max_size = 1024;
  unsigned int i;
  double a[max_size];
  SDTRandomSequence *sizes = SDTRandomSequence_newInt(8, 2, max_size);
  SDTRandomSequence *values = SDTRandomSequence_newFloat(0, random_min, random_max);
  FOR_RANDOM_ITER_INT(sizes, s)
  {
    for (i = 0; i < s; ++i)
    {
...
void TestSDT_rank(CuTest* tc)
{
  ...
  for (unsigned int t = 0; t < 8; ++t)
  {
    ...
    for (i = 0; i < size; ++i)
      CuAssertDblEquals_Msg(tc, "Check rank function", (double) i, SDT_rank(dranks, size, i), 0.0);
  }
...
void TestSDT_samplesInAir__SDT_samplesInAir_inv(CuTest* tc)
{
  ...
  FOR_RANDOM_ITER_FLOAT(values, f)
  {
    ...
    CuAssertDblEquals_Msg(tc, "Check linear dependency", k * s, SDT_samplesInAir(k * v), 0.000000001);
    CuAssertDblEquals_Msg(tc, "Check inverse", v, SDT_samplesInAir_inv(s), 0.000000001);
  }
  SDTRandomSequence_free(values);
  SDT_TEST_END()
}
...
void TestSDT_signum(CuTest* tc)
{
  ...
  FOR_RANDOM_ITER_FLOAT(values, v)
  {
    ...
    CuAssertIntEquals_Msg(tc, "Check consistency", (v > 0) ? 1 : -1, SDT_signum(v));
  }
  SDTRandomSequence_free(values);
...
void TestSDT_sinc__SDT_truePeakPos__SDT_truePeakValue(CuTest* tc)
{
  ...
  for (unsigned int s = max_size / 4; s < max_size; ++s)
  {
    ...
    CuAssertDblEquals_Msg(tc, "Peak in the middle", (s - 1.0) / 2.0, p, 0.000000001);
...
void TestSDT_weightedAverage(CuTest* tc)
{
  ...
  for (c = 0; c < n_cases; ++c)
  {
    ...
    CuAssertDblEquals_Msg(tc, "Test happy path", truth, SDT_weightedAverage(values, k_weights, n), tol);
  }

  double emptyValues[] = {};
  double emptyWeights[] = {};
  CuAssertDblEquals_Msg(tc, "Test edge case: empty arrays", 0.0, SDT_weightedAverage(emptyValues, emptyWeights, 0), 0.0);
  ...
  for (c = 0; c < n_cases; ++c)
  {
    ...
    {
      k_weights[i] = k * weights[i];
      CuAssertDblEquals_Msg(tc, "Test edge case: single value and weight", values[i], SDT_weightedAverage(values + i, k_weights + i, 1), tol);
    }
  }

  for (i = 0; i < n; ++i)
    k_weights[i] = 0.0;
  CuAssertDblEquals_Msg(tc, "Test edge case: all weights are zero", 0.0, SDT_weightedAverage(values, k_weights, n), 0.0);
  ...
  for (c = 0; c < n_cases; ++c)
  {
    ...
    for (i = 0; i < n; ++i)
      k_weights[i] = k * weights[i];
    CuAssertDblEquals_Msg(tc, "Test edge case: all values are zero", 0.0, SDT_weightedAverage(zeroValues, k_weights, n), 0.0);
  }

  CuAssertDblEquals_Msg(tc, "Test edge case: n is zero", 0.0, SDT_weightedAverage(values, weights, 0), 0.0);
