/** \file impact_2modalb.cpp
 * flext-based external for Pd and Max implementing interaction between two 
 * modal resonators by means of a non-linear impact force.
...
#define external_name "impact_2modalb~\0"
 ...
 */
typedef struct _impact_2modalb_ctl /**< Links the 2 interacting objects and the interactor */
{
	...
	t_pickup_signal_function_modalb *p_pickup_signal1, /**< Pointers to pickup functions for all interaction points of modal resonator */
									*p_pickup_signal2;
} t_impact_2modalb_ctl;


// A flext dsp external ("tilde object") inherits from the class flext_dsp
class impact_2modalb: 
	public flext_dsp	/* N.B. */
{	
	FLEXT_HEADER_S(impact_2modalb, flext_dsp, setup)

	public:
		//TODO con attributi 
		impact_2modalb(int argc, t_atom *argv);	// CONSTRUCTOR
		~impact_2modalb();	// DESTRUCTOR
		
	protected:
		float samprate;
		t_impact_2modalb_ctl x_cspace; 
		t_impact_2modalb_ctl *x_ctl;
	...
	//METHODS PROTOTYPES
		...
		void m_interactor_props(float k, float alpha, float lambda);
	...
	// for every registered method a callback has to be declared
		...
		FLEXT_CALLBACK_FFF(m_interactor_props)
...
}; // end of class declaration for impact_2modalb


FLEXT_NEW_DSP_V("impact_2modalb~", impact_2modalb)


/**** DEFINITIONS ****/

// ---- SETUP ----
void impact_2modalb::setup(t_classid c)
{
// --- set up methods (class scope) ---
	...
	FLEXT_CADDMETHOD_FFF(c, 0, "nteract", m_interactor_props);	// 3 float args
	...
	FLEXT_CADDMETHOD(c, 4, m_interactor_props);	// 3 float args recognized automatically
...
// ---- CONSTRUCTOR ----
impact_2modalb::impact_2modalb(int argc, t_atom *argv)
{	
    ...
    post("   %s: Nonlinear impact between 2 'modal' resonators.", external_name);
...
//** object2: modal object. "struck" **/
	...
	for (i=0; i<n_picks2; i++) //Output masks
		if (GetAFloat(argv[pos_object2+2+i]) == 1)
		{
...
//modal obj. 2 ("struck")
  ...
  pos_interacts2 = pos_ts2 + n_modes2;
	...
	AddInList("list[3]: interactor properties");			// 1 List in (5th inlet)
...
//interactor
	m_interactor_props(GetAFloat(argv[0]), GetAFloat(argv[1]), GetAFloat(argv[2]));		//k, alfa, lambda
...
// ---- DESTRUCTOR ----
impact_2modalb::~impact_2modalb()
{
	destroy_modal_object_b(x_ctl->p_object1);
	destroy_modal_object_b(x_ctl->p_object2);

	free(x_ctl->p_pickup_signal1);
	free(x_ctl->p_pickup_signal2);
}
 ...
 */
int impact_2modalb::prepare_modal_obj_b(int argc, t_atom *argv, int object_id)
{																
 ...
 */
void impact_2modalb::m_strike(float velocity)
{
  int i;
  ...
  //post("%s: Initial striker velocity is %f ", external_name, velocity);
}
 ...
 */	
void impact_2modalb::m_interactor_props(float k, float alpha, float lambda)
{
 ...
 */
void impact_2modalb::m_set_actmodes1(int n_actmodes)
{
 ...
 */
void impact_2modalb::m_base1(float base_freq0, float base_te, float base_gain)
{
  int i;
  ...
  set_Kmatrix_impactor(&x_ctl->felt, p_K_modalobjb(x_ctl->p_object1, INTERACT_PICK), p_K_modalobjb(x_ctl->p_object2, INTERACT_PICK) );
}

/**
 * Sets the undamped frequencies for the modes of modal object 1.
 */
void impact_2modalb::m_mode_freqs1(int argc, t_atom *argv)
{
  int i;
 ...
 */
void impact_2modalb::m_mode_ts1(int argc, t_atom *argv)
{
  int i;
 ...
 */
void impact_2modalb::m_mode_contribs1(int argc, t_atom *argv)
{
  float weight;
  int i;
 ...
 */
void impact_2modalb::m_set_actmodes2(int n_actmodes)
{
 ...
 * K-matrices are (re)passed to the interactor.
 */
void impact_2modalb::m_base2(float base_freq0, float base_te, float base_gain)
{
  int i;
 ...
 */
void impact_2modalb::m_mode_freqs2(int argc, t_atom *argv)
{
  int i;
 ...
 */
void impact_2modalb::m_mode_ts2(int argc, t_atom *argv)
{
  int i;
 ...
 */
void impact_2modalb::m_mode_contribs2(int argc, t_atom *argv)
{
  float weight;
  int i;
  int pick = GetAInt(argv[0]);
...
void impact_2modalb::m_signal(int n, float * const * in, float * const * out)
{
	...
	for(int si = 0; si < n; ++si)
	{
		...
		post(" ---- impactor:");
#endif
		impact_force_impactor(&force1, &force2, //Returns the interaction forces in "force1" and "force2" 
							  &x_ctl->felt,
							  displ,	//these are x~(n) for the impactor
							  displ_modalobjb(x_ctl->p_object2, INTERACT_PICK) + (x_ctl->last_distance = *ext_offset++),
							  velo,		//these are v~(n) for the impactor
