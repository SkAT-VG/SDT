/** \file friction_2modalb.cpp
 * flext-based external for Pd and Max implementing interaction between two
 * modal resonators by means of a non-linear friction force.
...
#define external_name "friction_2modalb~\0"
 ...
 */
typedef struct _friction_2modalb_ctl /**< Links the 2 interacting objects and the interactor */
{
  t_modal_object_b *p_object1, /**< Pointer to rubbing object ("rubber") */
  ...
  t_pickup_signal_function_modalb *p_pickup_signal1, /**< Pointers to pickup functions for all interaction points of modal resonator */
								*p_pickup_signal2;
} t_friction_2modalb_ctl;


// A flext dsp external ("tilde object") inherits from the class flext_dsp
class friction_2modalb: 
	public flext_dsp	/* N.B. */
{	
	FLEXT_HEADER_S(friction_2modalb, flext_dsp, setup)

	public:
		//TODO con attributi 
		// CONSTRUCTOR with no arguments
		// initial values are set by attributes at creation time (see help file)	/* N.B. */
		friction_2modalb(int argc, t_atom *argv);
		
	protected:
		float samprate;
		t_friction_2modalb_ctl x_cspace; 
		t_friction_2modalb_ctl *x_ctl;
...
}; // end of class declaration for friction_2modalb

FLEXT_NEW_DSP_V("friction_2modalb~", friction_2modalb)


/**** DEFINITIONS ****/

// ---- SETUP ----
void friction_2modalb::setup(t_classid c)
{
// --- set up methods (class scope) ---

	// set up tagged methods for the default inlet (0)
	FLEXT_CADDMETHOD_F(c, 0, "start_rubbing", m_strike);  // single float arg
...
// ---- CONSTRUCTOR ----
friction_2modalb::friction_2modalb(int argc, t_atom *argv)
{	
    ...
    post("   %s: Friction between 2 'modal' resonators.", external_name);
	...
	AddInSignal("signal: external rubbing force");			// 1 float in (1st inlet) //floats are translated to signals
	AddInSignal("signal: external force on rubbed object");		// 1 float in (2nd inlet)
	AddInSignal("signal: additional offset between objects");   // 1 float in (3th inlet)

	AddInFloat("float: initial rubber velocity");				// 1 float in (4th inlet)
	...
	for (i=0; i<n_picks1; i++) //1 outlet for each interaction points of modal obj1 
		AddOutSignal("signal: rubber output");
	for (i=0; i<n_picks2; i++) //1 outlet for each interaction points of modal obj2
		AddOutSignal("signal: rubbed object output"); 
 ...
 */
int friction_2modalb::prepare_modal_obj_b(int argc, t_atom *argv, int object_id)
{																
	...
	if (argc>0)
		...
		if ((n_modes = GetAInt(argv[0])) <= 0)
			{															
			post("%s: !!! n_modes of object%d <= 0 ?!! is set to 1.", external_name, object_id); 
			n_modes = 1;
			}
		if (argc>1)
			if ((n_picks = GetAInt(argv[1])) <= 0)
			{
			post("%s: !!! n_picks of object%d <= 0 ?!! is set to 1.", external_name, object_id);
			n_picks = 1;
			}
		}
...
/**
 * Sets "mode 0 (inertial mode) displ of the rubber" = "struck obj displ" (plus additional offset) --> contact!
 * Set objects to contact and "rubber" initial velocity.	
 * All modes except no. 1 are set to "0 state".	
 */
void friction_2modalb::m_strike(float velocity)
{
	int i;
	...
	//post("Initial rubbing velocity is %f ", velocity);
}
 ...
 */	
void friction_2modalb::m_interactor_props(int argc, t_atom *argv)
{
...
//OBJECT 1 methods (modal "rubber")

/**
 * Set the number of active modes for object 1.
 */
void friction_2modalb::m_set_actmodes1(int n_actmodes)
{
 ...
 */
void friction_2modalb::m_base1(float base_freq0, float base_te, float base_gain)
{
	int i;
	...
	set_Kmatrix_frictionor(&x_ctl->bow, p_K_modalobjb(x_ctl->p_object1, INTERACT_PICK), p_K_modalobjb(x_ctl->p_object2, INTERACT_PICK), samprate );
}

/**
 * Sets the undamped frequencies for the modes of modal object 1.
 */
void friction_2modalb::m_mode_freqs1(int argc, t_atom *argv)
{
    if(argc < x_ctl->p_object1->publ.n_modes)
        post("%s %s - number of arguments doesn't match",thisName(),GetAString(thisTag()));
    else {
        int i;
 ...
 */
void friction_2modalb::m_mode_ts1(int argc, t_atom *argv)
{
    if(argc < x_ctl->p_object1->publ.n_modes)
        post("%s %s - number of arguments doesn't match",thisName(),GetAString(thisTag()));
    else {
 ...
 */
void friction_2modalb::m_mode_contribs1(int argc, t_atom *argv)
{
    if(argc < x_ctl->p_object1->publ.n_modes+1)
        post("%s %s - number of arguments doesn't match",thisName(),GetAString(thisTag()));
    else {
        float weight;
        int i;
...
//OBJECT 2 methods ("rubbed")

/**
 * Set the number of active modes for object 2.
 */
void friction_2modalb::m_set_actmodes2(int n_actmodes)
{
 ...
 * K-matrices are (re)passed to the interactor.
 */
void friction_2modalb::m_base2(float base_freq0, float base_te, float base_gain)
{
	int i;
	...
	set_Kmatrix_frictionor(&x_ctl->bow, p_K_modalobjb(x_ctl->p_object2, INTERACT_PICK), p_K_modalobjb(x_ctl->p_object2, INTERACT_PICK), samprate );
}

/**
 * Sets the undamped frequencies for the modes of modal object 2.
 */
void friction_2modalb::m_mode_freqs2(int argc, t_atom *argv)
{
    if(argc < x_ctl->p_object2->publ.n_modes)
        post("%s %s - number of arguments doesn't match",thisName(),GetAString(thisTag()));
    else {
        int i;
 ...
 */
void friction_2modalb::m_mode_ts2(int argc, t_atom *argv)
{
    if(argc < x_ctl->p_object2->publ.n_modes)
        post("%s %s - number of arguments doesn't match",thisName(),GetAString(thisTag()));
    else {
 ...
 */
void friction_2modalb::m_mode_contribs2(int argc, t_atom *argv)
{
    if(argc < x_ctl->p_object2->publ.n_modes+1)
        post("%s %s - number of arguments doesn't match",thisName(),GetAString(thisTag()));
    else {
        float weight;
        int i;
...
void friction_2modalb::m_signal(int n, float * const * in, float * const * out)
{
	const float * ext_force1 = in[0];	/**< External force to be applied to the rubbing object */
    ...
    for(int si = 0; si < n; ++si)
	{
		...
		post("CYCLE # %d:", si);
#endif
		start_cycle_modalobjb(x_ctl->p_object1); //Computes the state of the "rubbing" obj. ignoring current forces
		start_cycle_modalobjb(x_ctl->p_object2); //Computes the state of the "rubbed" obj. ignoring current forces
		...
		friction_force_frictionor(&force1, &force2, //Returns the interaction forces in "force1" and "force2" 
							  &x_ctl->bow, 
							  velo_modalobjb(x_ctl->p_object1, INTERACT_PICK),	//these are v~(n) for the frictionor
