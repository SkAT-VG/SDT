...
#include <stdlib.h> //TODO this is needed only to use NULL...

#include "MeshStat.h"
#include <math.h>


Mesh :: Mesh(int nsources)
{
	int i;
	rate = 22050.;
	length = 100;
	width = 7;
	m_nsources = nsources;
	pos_sources = new float[m_nsources];
...
void Mesh :: setPosSources(float* sourPos)
{
	int i;
	for (i=0; i< m_nsources; i++)
	{
		pos_sources[i] = (sourPos[i]) / (sqrt(2)*343/rate);
	}
}

void Mesh :: setPosList(float listPos)
{
	pos_listener = listPos / (sqrt(2)*343/rate);
}
...
float Mesh :: computeSample(float* in)
{
  int i,xit,y;
  float intercoeff;
  ...
  for (i=0; i< m_nsources; i++)
  {
	  ximp = (int)(floor(pos_sources[i]));
	  yimp = (int)(floor(width/2));
	  
	  intercoeff = pos_sources[i] - floor(pos_sources[i]);  // de-interpolate input sound over two junctions if needed
	  IN3[ximp][yimp] = in[i]*(1-intercoeff);
      IN3[ximp+1][yimp] = in[i]*intercoeff;
   }
	  ...
	  outsamp=(floor(pos_listener)+1-pos_listener)*J[(int)(floor(pos_listener))][yimp] + (pos_listener-floor(pos_listener))*J[(int)(floor(pos_listener)+1)][yimp];
	  return outsamp;
}
